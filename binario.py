...
class binaryfile:
    ...
    def read_float(self,N=1,cast=np.float32):
        if N==1: return cast( struct.unpack('f',self.f.read(4))[0] )
        else: return cast( struct.unpack('{:d}f'.format(N),self.f.read(N*4))[0:N] )
